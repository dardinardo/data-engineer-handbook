...
SELECT
    ...
    -- comment, -- rimosso perché abbiamo messi questi sopra
    CAST(SPLIT_PART(min, ':', 1) AS REAL) * 60 + CAST(SPLIT_PART(min, ':', 2) AS REAL) AS m_minutes,
    min AS m_min,
    fgm AS m_fgm,
    fga AS m_fga,
    fg3m AS m_fg3m,
...
CREATE TABLE fct_game_details AS (
    dim_game_date DATE,
    dim_season INTEGER,
    dim_team_id INTEGER,
...
SELECT
    dim_player_name,
    -- COUNT(CASE WHEN dim_did_not_play THEN 1 END) AS did_not_play,
    -- COUNT(CASE WHEN dim_did_not_dress THEN 1 END) AS did_not_dress,
