...
def create_session_aggregated_events_referrer_sink_postgres(t_env):
    table_name = 'processed_session_events_aggregated_source'
    sink_ddl = f"""
        CREATE TABLE {table_name} (
            event_hour TIMESTAMP(3),
            ip VARCHAR,
...
def log_aggregation():
    ...
    try:
        ...
        aggregated_table = create_session_aggregated_events_sink_postgres(t_env)
        aggregated_sink_table = create_session_aggregated_events_referrer_sink_postgres(t_env)
        t_env.from_path(source_table)\
            ...
            .select(
                    ...
                    col("ip"),
                    col("ip").count.alias("num_hits_from_ip")
            ) \
